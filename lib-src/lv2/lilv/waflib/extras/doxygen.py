...
class doxygen(Task.Task):
	...
	def runnable_status(self):
		...
		if not getattr(self, 'pars', None):
			...
			if self.pars.get('OUTPUT_DIRECTORY'):
				...
				output_node = (self.inputs[0].parent.get_bld().make_node(
				    f'{self.inputs[0].name}.doxy'))
			output_node.mkdir()
	...
	def add_install(self):
		nodes = self.output_dir.ant_glob('**/*', quiet=True)
		self.outputs += nodes
		if getattr(self.generator, 'install_path',
		           None) and not getattr(self.generator, 'doxy_tar', None):
